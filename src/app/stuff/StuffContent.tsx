    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
        {links.map((link, index) => (
          <div key={index} className="p-4 sm:p-6 rounded-xl transition-all duration-300 transform hover:-translate-y-2 hover:shadow-lg bg-[var(--card-bg-color)] shadow-[var(--card-shadow)] flex flex-col justify-between border border-[var(--gray-200)] dark:border-[var(--gray-700)] h-full">
            <div>
              <h3 className="text-base sm:text-lg font-semibold mb-2 text-[var(--text-color)]">{link.title}</h3>
              <p className="mb-4 text-xs sm:text-sm text-[var(--text-color)]">{link.description}</p>
            </div>
            <a 
              href={link.url} 
              target="_blank" 
      ...
      <div className="p-4 sm:p-6 rounded-xl text-center mb-8 sm:mb-12 transition-all duration-300 transform hover:-translate-y-2 hover:shadow-lg bg-[var(--card-bg-color)] shadow-[var(--card-shadow)] border border-[var(--gray-200)] dark:border-[var(--gray-700)]">
        <h3 className="text-lg sm:text-xl font-semibold mb-2 text-[var(--title-color)]">My MC Server (Survival)</h3>
        <p className="mb-3 text-xs sm:text-sm text-[var(--text-color)]">Click below to copy the server IP:</p>
        <button 
      ...
      <h2 className="text-xl sm:text-2xl font-bold text-center my-6 sm:my-8 text-[var(--title-color)]">Telegram Channels/Groups</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
